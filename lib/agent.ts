...
export function localDecompose(
  ...
  if (lines.length)
    return lines.map((line) => {
      const m = line.match(/(\d+)\s*(?:分钟|min)/i);
      return {
        title: line
          .replace(/[（(]?\d+\s*(?:分钟|min)[)）]?/i, '')
          .trim()
          .slice(0, 160),
        outcome: '完成该项内容并检查结果',
  ...
  return [
    {
      title: `明确「${title}」的具体步骤`,
      outcome: '列出需要完成的内容与验收清单',
      minutes: 15,
      energy: 'low',
      dependsOn: [],
    },
    {
      title: `推进「${title}」的第一份成果`,
      outcome: outcome || '产出可检查的初稿',
      minutes: 50,
      energy: 'high',
      dependsOn: [0],
    },
...
export function validateDraftTasks(value: unknown): DraftTask[] {
  if (!Array.isArray(value) || value.length < 1 || value.length > 30)
    throw new Error('未获得有效任务清单，请重试或手动添加。');
  return value.map((raw: unknown, index) => {
    ...
    if (
      ...
      throw new Error('生成的任务格式不完整，请重试。');
